...
# We have connection draining set to 300 seconds for production ELBs,
# so if any traffic does get stuck for a while, this is going to cause a failed deployment.
# Unless our wait attempts here is greater the the connection draining setting.
WAITER_ATTEMPTS=360
